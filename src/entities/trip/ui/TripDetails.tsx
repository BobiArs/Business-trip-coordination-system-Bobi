...
export default function TripDetails({
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
      {/* Ліва колонка: тут буде основна інформація та коментарі */}
      <div className="lg:col-span-2 space-y-6">
        <div className="bg-white dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800 p-6 md:p-8 rounded-2xl shadow-sm space-y-6">
          <div className="flex flex-wrap items-start justify-between gap-4 pb-4 border-b border-slate-100 dark:border-slate-800/80">
            ...
            <span
              className={`text-xs font-bold px-3 py-1.5 rounded-full border ${getStatusBadgeClass(trip.statusId)}`}
            >
              {statusName}
            </span>
          </div>
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <span
                    className={`inline-flex items-center text-xs font-semibold px-2 py-0.5 rounded border mt-0.5 ${getUrgencyTextClass(trip.urgencyId)}`}
                  >
                    {urgencyName}
                  </span>
                </div>
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <span className="text-xs text-slate-400 dark:text-slate-500 block">
                    {trip.employeeEmail} · {trip.employeePhone}
                  </span>
                </div>
              </div>
          ...
          {comments.length === 0 ? (
            <div className="text-center py-6 border border-dashed border-slate-200 dark:border-slate-800 rounded-xl bg-slate-50/20">
              <p className="text-sm text-slate-400 dark:text-slate-500">
                До цієї заявки немає коментарів
              </p>
            </div>
          ) : (
            <div className="space-y-4">
              {comments.map((c) => (
                <div
                  key={c.id}
                  className="flex gap-3 items-start bg-slate-50/50 dark:bg-slate-950/40 border border-slate-100 dark:border-slate-800/80 p-4 rounded-2xl"
                >
      ...
      {/* Права колонка: в ній історія хронології */}
      <div className="bg-white dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800 p-6 rounded-2xl shadow-sm self-start space-y-6">
        ...
        {history.length === 0 ? (
          ...
          <div className="relative pl-6 space-y-6 border-l-2 border-slate-100 dark:border-slate-800/80 ml-3 py-1">
            {history.map((h) => {
              const oldStatusName =
                statuses.find((s) => s.id === h.oldStatusId)?.name ||
                h.oldStatusId ||
                "—";
              const newStatusName =
                statuses.find((s) => s.id === h.newStatusId)?.name ||
                h.newStatusId;
                ...
                <div key={h.id} className="relative group">
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="text-[10px] text-slate-400 dark:text-slate-500">
                      <span>
                        Оновив:{" "}
                        <strong className="font-semibold">{h.updatedBy}</strong>
                      </span>
