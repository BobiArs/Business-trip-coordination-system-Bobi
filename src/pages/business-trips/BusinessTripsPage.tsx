...
// Сторінка для відображення списку заявок
export default function BusinessTripsPage() {
  const { role } = useAuth();
  const [searchParams, setSearchParams] = useSearchParams();
  const navigate = useNavigate();
  const { addToast } = useUI();
  // Отримуємо параметр фільтрації зі URL
  const statusFilter = searchParams.get("status") || "";

  // Основний запит для отримання заявок
  const {
    data: trips,
    isLoading,
    isError,
  ...
  // Запит для отримання списку статусів
    ...
    return (
      <div className="text-center py-16 bg-white dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800 rounded-2xl p-8 max-w-md mx-auto shadow-sm">
        ...
        <p className="text-slate-500 dark:text-slate-400 text-sm mb-6">
          Невдалося підключитися до сервера. Спробуйте оновити сторінку.
        </p>
        <button
  ...
  // Фільтруємо заявки на клієнті за обраним статусом
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        {/* Кнопка створення заявки для співробітника */}
        {role === "employee" && (
          <button
            onClick={() => {
      ...
      <div className="flex items-center gap-3 bg-white dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800/80 p-4 rounded-2xl shadow-sm">
        ...
        <select
          value={statusFilter}
          onChange={(e) => {
            // Оновлюємо search-параметр в URL при зміні
      ...
      {/* Відображення, якщо заявок не знайдено */}
